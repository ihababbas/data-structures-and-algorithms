...
const longestString = (arr) => {
  ...
  ans = arr.reduce((acc, cur) => {
    cur.length>acc.length ? acc=cur : acc=acc;
    return acc
    })
  }
...
const firstLetters = (arr) => {   return arr.map(i => i.charAt(0))
  // Solution code here...
};
...
const findHappiness = (arr) => {  return arr.filter(i => i.includes(`:)`))
  // Solution code here...
};
...
const standardizePhoneNumbers = (arr) => { return arr.map(i => i.replace(/\D/g, ''))
  // Solution code here... 
};
...
const onlyOddChars = (str) => {let ans = [];
  str.split('').reduce((acc, val, idx) => idx%2===1? ans.push(val): '', [])
  return ans.join('')
  // Solution code here...
};
...
const allHappy = (arr) => {  return arr.length===arr.filter(x => x.includes(`:)`)).length
  // Solution code here... 
};
