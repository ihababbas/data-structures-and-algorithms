...
function transformToLis(obj){   return Object.keys(obj).map(key => `<li>${key}: ${obj[key]}</li>`)
  // Solution code here...
};
...
const count = (target, input) => {    let ans = 0;
  input.map(arr => ans += arr.filter(num => num === target).length)
  return ans
  // Solution code here...
};
...
const totalSum = (input) => {   let sum = 0;
  input.map(arr => sum += arr.reduce((acc, num) => acc += num, 0))
  return sum
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => {  let ans = input.map(arr => arr.filter(i => i%5===0))
  ans = ans.map(arr => arr.filter(i => typeof i === 'number'))
  return ans.map(arr => arr.map(i => 2**i))
  // Solution code here...
};
...
let findMaleAndFemale = (data) => { 
  ...
  data.forEach(character => {
    (character.gender === 'male' || character.gender === 'female') ? ans.push(character.name): '';
  })
  ans = ans.join(' and ')
  return ans
  // Solution code here...
...
let findShortest = (data) => {
  let heights = data.map(x => x.height)
  heights.sort((a,b) => a-b)
  let ans = data.filter(x => x.height.includes(heights[0]))
  return ans[0].name
  // Solution code here...
};
